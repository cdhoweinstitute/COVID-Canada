function makeLineChart(dataset, xName, yObjs, axisLables) {
    var chartObj = {};
    var color = d3.scale.ordinal().range(["#00548b", "#002e56", "#bea25b", "#dac9a2"]);
...
//Formatter functions for the axes
    ...
    chartObj.formatAsPercent = d3.format("");
    chartObj.formatAsDate = d3.time.format("%b %d")
    chartObj.xFormatter = chartObj.formatAsNumber;
    chartObj.yFormatter = chartObj.formatAsPercent;
...
//Create scale functions
    ...
    console.log(d3.extent(chartObj.data, chartObj.xFunct))
// Get the max of every yFunct
    ...
    chartObj.yScale = d3.scale.linear().range([chartObj.height, 0]).domain([d3.min(chartObj.yFuncts.map(chartObj.min)), d3.max(chartObj.yFuncts.map(chartObj.max))]);

    chartObj.formatAsYear = d3.format("");

    chartObj.xAxis = d3.svg.axis().scale(chartObj.xScale).orient("middle").tickFormat(chartObj.xFormatter); //< Can be overridden in definition
